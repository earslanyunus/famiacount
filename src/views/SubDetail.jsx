      ...
      <div className="modal">
        <div className="modal-box">
          ...
          <div className="modal-action">
            ...
            <label onClick={()=>confirmPayment({userId:item.uid,subscriptionId:id})} htmlFor="edit-modal" className="btn">
              Accept
            </label>
          </div>
        </div>
          ...
          <div className="modal-action">
            ...
            <label onClick={()=>confirmPayment({userId:item.uid,subscriptionId:id})} htmlFor="edit-modal" className="btn">
              ...
              <div key={item.uid} className={`flex gap-3  mt-4  `}>
                <img className="rounded-full h-12" src={item.photoURL} alt="" />
                <div>
                  <p className="text-gray-700 font-bold text-left">{item?.displayName}</p>
                  <p className="text-red-500">{!user.isPayed && 'Payment Issue'}</p>
                </div>
